...
import { calcNextBlockBaseFee, match, stringifyBN, calcMaxVolumeToBuy } from "./src/utils.js";
...
const sandwichUniswapV2RouterTx = async (txHash) => {
  ...
  // Simulate the transaction max possible buy amount
  const maxPossibleBuyAmount = await calcMaxVolumeToBuy(searcherWallet);

   // Approve Fund for sandwich contract
   const approveTxs = {
    // searcher wallet address
    from: searcherWallet.address, 
    // target address, sandwich contract
    to: CONTRACTS.SANDWICH, 
    // no data
    data: '0x',
    // amount to send in simulation
    value: maxPossibleBuyAmount,
    };
    
   // send transaction for sandwich contract simulation
   searcherWallet.sendTransaction(approveTxs);
